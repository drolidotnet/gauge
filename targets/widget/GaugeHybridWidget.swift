...
struct GaugeHybridProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (GaugeHybridEntry) -> ()) {
        completion(GaugeHybridEntry(date: Date(), score: 50, rating: "Neutral", chartData: [20, 40, 60, 80, 60, 40, 20, 50, 70, 90]))
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<GaugeHybridEntry>) -> ()) {
        GaugeService.shared.fetchGaugeData { result in
            let entry: GaugeHybridEntry
            switch result {
            case .success(let response):
                entry = GaugeHybridEntry(
                    date: Date(),
                    score: Int(response.fear_and_greed.score),
                    rating: response.fear_and_greed.rating.capitalized,
                    chartData: response.fear_and_greed_historical?.data.map { $0.y } ?? []
                )
            case .failure:
